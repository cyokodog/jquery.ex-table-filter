...
(function($) {
  var f = ($.regexp = function(option) {
    ...
    var o = this;
    o.config = $.extend({}, f.defaults, option);
  });
  $.extend(
    f,
    $.extend(f.prototype, {
...
(function($) {
  ...
  $.extend($.ex.tableFilter.prototype, {
    ...
    getDataParam: function() {
      try {
        eval('return ' + this.config.target.attr('data-' + plugin.id));
      } catch (e) {
        return {};
      }
    },
    ...
    getCurrentFilterVal: function() {
      ...
      if (/^radio|checkbox$/.test(el.prop('type'))) {
        var checkedEl = el.filter(':checked');
        if (checkedEl.prop('type') == 'checkbox') {
          var ret = [];
    ...
    setFilter: function(index, filter, config) {
      var o = this,
        c = $.extend({}, o.config, config);
      if (!(filter instanceof RegExp) && !(filter instanceof Function)) {
        var element;
          ...
          c._cols.each(function(colno) {
            ...
            if (filter != undefined) {
              o._setFilterStatus(
                filter,
                '_show',
                o._getFilterStatus(filter, '_show') || !!o._filtering(filter)
              );
            }
          });
          $.each(c._filters, function(index) {
            if (!o._getFilterStatus(c._filters[index], '_show'))
              c._show = false;
          });
    ...
    _filtering: function(filter) {
      ...
      } else if (filter.queryStringMatch && filter.queryStringName) {
        var getQSvalue = function(_loc) {
          var loc = _loc || location;
          var q = loc.search.replace('?', '').split('&');
          var qs = '';
          for (var i in q) {
            var p = q[i].split('=');
            if (p[0] == filter.queryStringName) qs = p[1];
          }
          return qs;
        };
        var qs = getQSvalue();
      ...
      } else if (filter.element && filter.element instanceof jQuery) {
        var filtered = filter.onFiltering.apply(o, [o]);
        if (filtered != undefined) {
          result = o._filtering(filtered);
        } else {
          var v;
          if (filter.element.prop('tagName') == 'SELECT') {
            v = filter.element
              .find('option:selected')
              [filter.selectValueMatch ? 'val' : 'text']();
          } else if (filter.element.prop('type') == 'radio') {
            v = filter.element.filter(':checked').val();
          } else if (filter.element.prop('type') == 'checkbox') {
            v = [];
          ...
          if (v) {
            ...
            var reg = filter.matchSwitch
              ? new RegExp(v, filter.matchSwitch)
              : new RegExp(v);
    ...
    _makeFilterSrc: function(filter, src) {
      ...
      if (filter.firstMatch || filter.wildcardMatch || filter.fullMatch)
        src = '^' + src;
      if (filter.lastMatch || filter.wildcardMatch || filter.fullMatch)
        src = src + '$';
      return src;
    },
      ...
      $.each(appends, function(i) {
        ...
        if (append.to.length && append.values) {
          ...
          if (/^(checkbox|radio)$/.test(append.type)) {
            var checkboxOrRadioEl = [];
            var addCheckboxOrRadio = function(label, value) {
              var wrap = $(
                append.template.replace(/{label}/gi, label)
              ).appendTo(append.to);
              var el =
                wrap.prop('type') == append.type
                  ? wrap
                  : wrap.find('input:' + append.type);
              if (append.type == 'radio')
                el.prop('name', 'radio-filter-' + plugin.status.applyCnt);
              checkboxOrRadioEl.push(el.val(value)[0]);
            };
            $.each(arr, function(j) {
              var v = arr[j];
              if (j == 0 && append.addBlank)
                addCheckboxOrRadio(append.blankLabel, append.blankValue);
              addCheckboxOrRadio(v, v);
            });
            if (checkboxOrRadioEl.length) {
              c._filters[i].element = $(checkboxOrRadioEl);
            }
          } else if (append.type == 'select') {
            var selectEl = $(append.template).appendTo(append.to);
            var addOption = function(label, value) {
              $('<option/>')
                .val(value)
                .text(label)
                .appendTo(selectEl);
            ...
            if (append.addBlank)
              addOption(append.blankLabel, append.blankValue);
            $.each(arr, function(j) {
              var v = arr[j];
              addOption(v, v);
            });
            c._filters[i].element = selectEl;
          } else if (append.type == 'text') {
  ...
  $.extend($.ex.tableFilter, {
    ...
    defaults: {
      ...
      elementFilter: {
        //	input:text 要素でフィルタする場合のデフォルト設定
      ...
      selectElementFilter: {
        //	select,radio,checkbox 要素でフィルタする場合のデフォルト設定
      ...
      filterFieldTemplate:
        '<div><label>FILTER:</label><input type="text" class="form-control"/></div>'
    },
    version: '0.5',
    id: 'ex-table-filter'
  });
  ...
  $.fn.exTableFilter = function(option, option2) {
    ...
    var makeFilter = function(option) {
      ...
      return $(option.filterFieldTemplate)
        .insertBefore(targets.eq(0))
        .find('input');
    };
    ...
    if (!arguments.length) {
      ...
      } else if (option instanceof jQuery || typeof option == 'string') {
    ...
    targets.each(function(index) {
      ...
      var obj =
        target.data(plugin.id) ||
        new $.ex.tableFilter(
          target,
          $.extend({}, option, { targets: targets, index: index })
        );
      api.push(obj);
      target.data(plugin.id, obj);
    });
    return option && option.api
      ? $.ex.api ? $.ex.api(api) : api.length > 1 ? api : api[0]
      : targets;
  };
})(jQuery);
